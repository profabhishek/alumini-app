...
<div class="ec-page">
    ...
    <div class="ec-header">
        <div>
            <h1 class="ec-title">Event Categories</h1>
            <p class="ec-subtitle">Manage categories used when creating events</p>
        </div>
    ...
    {{-- ── Search & Filter ─────────────────────────────────────────────── --}}
    <form method="GET" action="{{ route('admin.event-categories.index') }}" class="ec-filters">
        <input
            type="text"
            name="q"
            value="{{ request('q') }}"
            placeholder="Search categories..."
            class="ec-input ec-search"
        >
        ...
        @if(request('q') || request('status'))
            <a href="{{ route('admin.event-categories.index') }}" class="ec-btn ec-btn-ghost">Clear</a>
        @endif
    </form>
            ...
            <tbody>
                @forelse($categories as $category)
                    <tr id="row-{{ $category->id }}">
                        ...
                        <td>
                            <span class="ec-badge {{ $category->is_active ? 'ec-badge-active' : 'ec-badge-inactive' }}"
                        ...
                        <td>
                            <div class="ec-actions">
                                {{-- Edit --}}
                                <button class="ec-icon-btn ec-icon-edit" title="Edit"
                                ...
                                {{-- Toggle --}}
                                <button class="ec-icon-btn ec-icon-toggle" title="Toggle Status"
                                ...
                                {{-- Delete --}}
                                <button class="ec-icon-btn ec-icon-delete" title="Delete"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="ec-empty">No categories found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="ec-modal-overlay" id="createModal" onclick="closeModal('createModal')">
    <div class="ec-modal" onclick="event.stopPropagation()">
        <div class="ec-modal-header">
            <h2 class="ec-modal-title">New Category</h2>
        ...
        <form id="createForm">
            ...
            <div class="ec-modal-body">
                <div class="ec-form-group">
                    ...
                    <input type="text" name="name" id="create_name" class="ec-input" placeholder="e.g. Workshop" required>
...
<div class="ec-modal-overlay" id="deleteModal" onclick="closeModal('deleteModal')">
    <div class="ec-modal ec-modal-sm" onclick="event.stopPropagation()">
        ...
        <div class="ec-modal-body">
            <p class="ec-confirm-text">Are you sure you want to delete <strong id="deleteModalName"></strong>? This action cannot be undone.</p>
        </div>
        <div class="ec-modal-footer">
...
/* Buttons */
.ec-btn { display: inline-flex; align-items: center; gap: .4rem; padding: .5rem 1rem; border-radius: 8px; font-size: .875rem; font-weight: 500; cursor: pointer; border: none; text-decoration: none; transition: background .15s, opacity .15s; }
...
.ec-icon-btn { display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; border-radius: 6px; border: 1px solid #e5e7eb; background: #fff; cursor: pointer; transition: background .15s, border-color .15s; color: #6b7280; }
.ec-icon-btn:hover { background: #f3f4f6; border-color: #d1d5db; }
.ec-icon-delete:hover { background: #fef2f2; border-color: #fecaca; color: #dc2626; }
.ec-icon-edit:hover   { background: #eff6ff; border-color: #bfdbfe; color: #2563eb; }
...
const BASE   = '{{ url("/admin/event-categories") }}';
...
document.getElementById('createForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (res.ok && json.success) {
            ...
            if (json.errors) showErrors('create', json.errors);
            else showToast(json.message || 'Something went wrong.', 'error');
        }
    } catch {
...
document.getElementById('editForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (res.ok && json.success) {
            ...
            if (json.errors) showErrors('edit', json.errors);
            else showToast(json.message || 'Something went wrong.', 'error');
        }
    } catch {
        showToast('Request failed.', 'error');
...
async function toggleCategory(id) {
    try {
        ...
        if (res.ok && json.success) {
            ...
            if (badge) {
                badge.textContent = json.is_active ? 'Active' : 'Inactive';
                badge.className   = 'ec-badge ' + (json.is_active ? 'ec-badge-active' : 'ec-badge-inactive');
            }
        } else {
            showToast(json.message || 'Toggle failed.', 'error');
        }
    } catch {
        showToast('Request failed.', 'error');
    }
