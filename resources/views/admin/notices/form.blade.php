...
<div class="admin-page">
    ...
    <div class="admin-page-header">
        <div>
            <h1 class="admin-page-title">{{ $isEdit ? 'Edit Notice' : 'Create Notice' }}</h1>
            <p class="admin-page-subtitle">{{ $isEdit ? $notice->title : 'Publish a new announcement for alumni.' }}</p>
        </div>
        <a href="{{ route('admin.notices.index') }}" class="admin-link-btn">
    ...
    <form action="{{ $isEdit ? route('admin.notices.update', $notice) : route('admin.notices.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="admin-content-main">
                ...
                <div class="admin-card">
                    <div class="admin-form-group">
                        ...
                        <div class="admin-editor-wrap @error('description') is-error @enderror">
                            <div id="descriptionEditor"></div>
                        </div>
                        <textarea name="description" id="descriptionInput" hidden>{{ old('description', $notice->description) }}</textarea>
                    ...
                    <div class="admin-aside-group">
                        ...
                        <div class="admin-status-toggle">
                            <input type="radio" name="status" value="draft" id="statusDraft"
                                   {{ old('status', $notice->status) === 'draft' ? 'checked' : '' }}>
                            <label for="statusDraft" class="{{ old('status', $notice->status) === 'draft' ? 'is-active' : '' }}">Draft</label>

                            <input type="radio" name="status" value="published" id="statusPublished"
                                   {{ old('status', $notice->status) === 'published' ? 'checked' : '' }}>
                            <label for="statusPublished" class="{{ old('status', $notice->status) === 'published' ? 'is-active' : '' }}">Published</label>
                        </div>
                ...
                <div class="admin-card">
                    <div class="admin-aside-group">
                        ...
                        <div class="admin-category-select-row">
                            <select name="notice_category_id" class="admin-input" data-category-select>
                                <option value="">Uncategorized</option>
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ old('notice_category_id', $notice->notice_category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        ...
                        @error('notice_category_id')<span class="admin-form-error">{{ $message }}</span>@enderror
                    </div>
                </div>

                <div class="admin-card">
                    <div class="admin-aside-group">
                        <label>Image <span class="admin-form-hint">(optional)</span></label>

                        <div class="admin-image-upload" {{ $notice->image ? 'hidden' : '' }}>
                        ...
                        <div class="admin-image-preview-wrap" {{ $notice->image ? '' : 'hidden' }}>
                            @if($notice->image)
...
document.addEventListener('DOMContentLoaded', function () {
    AdminContent.initEditor('#descriptionEditor', '#descriptionInput');
